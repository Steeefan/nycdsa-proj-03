setwd('/Users/Daniel/Desktop/NYCDSA_Data')

library(dplyr)
library(xgboost)
library(tidyr)

apartments_train = readRDS('train-v10.rds')
apartments_test = readRDS('test-V10.rds')
...
xgb_train = xgb.cv(params, apartments_train_data_matrix, nrounds = 1000, nfold = 5, early_stopping_rounds = 20)
...
trained_xgb <- xgb.train(params = params, data=apartments_train_data_matrix, nrounds=nround)
...
write.csv(test_predictions,paste0('kaggle_apartments_xgboost ', Sys.time(),".csv"),row.names = FALSE)
